...
// Génère un numéro membre automatique : M-0001, M-0002, ...
  ...
  try {
    ...
    const sql = `
      SELECT m.*, u.nom, u.prenom, u.email, u.statut AS statut_compte,
        (SELECT statut_paiement FROM cotisations WHERE id_membre = m.id_membre AND annee = YEAR(NOW()) LIMIT 1) AS cotisation_annee
      FROM membres m
    ...
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM membres m JOIN utilisateurs u ON m.id_utilisateur = u.id_utilisateur WHERE ${where.join(' AND ')}`,
      params
    )
...
const getMembreById = async (req, res) => {
  try {
    const [rows] = await pool.query(
      `SELECT m.*, u.nom, u.prenom, u.email FROM membres m
       JOIN utilisateurs u ON m.id_utilisateur = u.id_utilisateur
       WHERE m.id_membre = ?`,
      [req.params.id]
    )
...
const createMembre = async (req, res) => {
  ...
  try {
    ...
    // Créer l'utilisateur avec rôle "membre"
