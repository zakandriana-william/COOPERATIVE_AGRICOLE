...
const getMembres = async (req, res) => {
  try {
    ...
    const sql = `
      SELECT m.*,
        (SELECT CASE WHEN COUNT(*) > 0 THEN 'payé' ELSE 'en_attente' END
         FROM cotisations WHERE membre_id = m.id AND annee = YEAR(NOW())) AS cotisation_annee
      FROM membres m
    ...
    const [[{ total }]] = await pool.query(
      `SELECT COUNT(*) AS total FROM membres m WHERE ${where.join(' AND ')}`, params
    )
